...
for det in dets:
    ...
    if make_plots:
        ...
        ut.plotfun(time, timemax, SNR, pcolor, det, eventname,
                    plottype, tevent, strain_whitenbp, template_match, 
                    template_fft, d_eff, freqs, data_psd, datafreq, fs)
...
# write the files:
ut.write_wavfile('audio/'+eventname+"_H1_whitenbp.wav",int(fs), strain_H1_whitenbp[indxd])
ut.write_wavfile('audio/'+eventname+"_L1_whitenbp.wav",int(fs), strain_L1_whitenbp[indxd])
...
ut.write_wavfile('audio/'+eventname+"_template_whiten.wav",int(fs), template_p_smooth[indxt])
...
# write the files:
ut.write_wavfile('audio/'+eventname+"_H1_shifted.wav",int(fs), strain_H1_shifted[indxd])
ut.write_wavfile('audio/'+eventname+"_L1_shifted.wav",int(fs), strain_L1_shifted[indxd])

# and the template:
template_p_shifted = ut.reqshift(template_p_smooth,fshift=fshift,sample_rate=fs)
ut.write_wavfile('audio/'+eventname+"_template_shifted.wav",int(fs), template_p_shifted[indxt])
